(function(a, c) {
    var b = function(e, d) {
        ...
        if(location.href.indexOf('alias=category_properties')!=-1){
            this.$element.on("click", ".tree-item>div", a.proxy(function(f) {
                var itemDOM = $(f.currentTarget).parent();
                if(!itemDOM.hasClass("tree-selected") && !$(f.currentTarget).hasClass("tree-folder-name")){
                    this.selectItem(itemDOM[0]);
                    FW.trigerEvent('trigerChangeClass',itemDOM.find("div[cid]").attr("cid"),itemDOM.find("div[cid]").attr("ctalias"));
                }else{
                    itemDOM.prev().trigger("click");
                }
            },
            this));
        }else{
