...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `wm_feedback_view` AS select `feedback`.`content` AS `content`,`user`.`fullname` AS `user_name`,`feedback`.`alias` AS `alias`,`user`.`phone` AS `user_phone`,`user`.`cid` AS `user_id`,`feedback`.`opertime` AS `opertime`,`feedback`.`cid` AS `cid`,`feedback`.`dataOwner` AS `dataOwner`,`feedback`.`nodeid` AS `nodeid` from (`wm_feedback` `feedback` left join `wm_users` `user` on((`feedback`.`user_id` = `user`.`cid`)));
